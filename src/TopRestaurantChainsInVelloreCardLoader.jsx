const TopRestaurantChainsInVelloreCardLoader = () => {
    return (
      <div className="flex flex-col space-y-2 p-4 w-full h-full shimmer rounded-xl">
        <div className="h-32 w-full shimmer rounded-lg"></div>
        <div className="h-4 w-3/4 shimmer rounded-md"></div>
        <div className="h-4 w-1/2 shimmer rounded-md"></div>
        <div className="h-4 w-full shimmer rounded-md"></div>
        <div className="h-4 w-1/2 shimmer rounded-md"></div>
      </div>
    );
  };
  
  export default TopRestaurantChainsInVelloreCardLoader;
  